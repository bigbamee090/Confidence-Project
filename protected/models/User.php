...
class User extends \app\components\TActiveRecord implements \yii\web\IdentityInterface
{
    ...
    const ROLE_CLINIC_ADMIN = 3;

    const ROLE_CLINIC_MANAGER = 4;

    const ROLE_PRESCRIBERS = 5;

    const ROLE_PATIENTS = 6;
    ...
    public static function getRoleOptions($id = null)
    {
        $list = array(
            ...
            self::ROLE_CLINIC => "Company",
            self::ROLE_CLINIC_ADMIN => "Company Admin",
            self::ROLE_CLINIC_MANAGER => "Company Manager",
            self::ROLE_PRESCRIBERS => "Prescribers"
            // self::ROLE_PATIENTS => "Patient"
        );
    ...
    public static function getCompanyRoleOptions()
    {
        return [
            self::ROLE_CLINIC_ADMIN => "Company Admin",
            self::ROLE_CLINIC_MANAGER => "Company Manager",
    ...
    public static function isCompanyAdmin()
    {
        ...
        return ($user->isActive() && $user->role_id == User::ROLE_CLINIC_ADMIN);
    }

    public static function isCompanyManager()
    {
        $user = Yii::$app->user->identity;
        if ($user == null)
            return false;
        
        return ($user->isActive() && $user->role_id == User::ROLE_CLINIC_MANAGER);
    }

    public static function isCompanyPrescriber()
    {
        $user = Yii::$app->user->identity;
        if ($user == null)
            return false;
        
        return ($user->isActive() && $user->role_id == User::ROLE_PRESCRIBERS);
    }
    ...
    {
        return (\Yii::$app->user->id == $id) ? "disabled = 'true'" : '';
    }
    ...
    public function getProfileImage($id)
    {
        ...
        if (! empty($id)) {
            ...
            if (! empty($user)) {
                ...
                $profileImage = $user->profile_file;
            }
        } else {
            $user = Yii::$app->user->identity;
            $profileImage = $user->profile_file;
        }
        $image_path = UPLOAD_PATH . $profileImage;
        if (! isset($user->profile_file) || ! file_exists($image_path))
            throw new NotFoundHttpException(Yii::t('app', "File not found"));
    ...
    {
        return $this->hasOne(CompanyAdmin::className(), [
            'user_id' => 'id'
        ]);
    }
    ...
    public function getCompanyPrescriber()
    {
        return $this->hasOne(CompanyPrescriber::className(), [
            'user_id' => 'id'
        ]);
    }
}