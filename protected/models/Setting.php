...
class Setting extends \app\components\TActiveRecord
{
    ...
    public static function getDefault($key)
    {
        ...
        if (isset($setting['core'][$key])) {
            return $setting['core'][$key];
        } elseif (isset($setting['custom'][$key])) {
            return $setting['custom'][$key];
        } else {
            ...
            if (isset($field['type'])) {
                switch ($field['type']) {
                   case self::KEY_TYPE_BOOL:
                        $html .= "<input type='checkbox' " . $required . " value='" . $value . "' class='form-control' name='Setting[keyValue][" . $key . "][value]'>";
